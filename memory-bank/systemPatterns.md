...
- **Images:**
  - **Ingestion:** Images are handled via a **background processing pattern**. The client provides an external URL to the `create` mutation, which immediately saves the album. A scheduled Convex Action (`storeCoverArt`) then runs in the background to download the image, store it in Convex Storage, and update the album record.
